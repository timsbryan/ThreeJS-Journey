...
void main() {
    ...
    // Pattern 15
    // float barX = step(0.2, mod(vUv.x * 10.0, 1.0));
    // barX *= step(0.8, mod(vUv.y * 10.0 + 0.3, 1.0));
    ...
    // Pattern 22
    // float bands1 = floor(vUv.x * 10.0) / 10.0;
    // float bands2 = floor(vUv.y * 10.0) / 10.0;
    // float strength = bands1 * bands2;
    ...
    //Coloured version
    vec3 blackColour = vec3(0.0);
    vec3 uvColour = vec3(vUv, 1.0);
    vec3 mixedColour = mix(blackColour, uvColour, strength);
